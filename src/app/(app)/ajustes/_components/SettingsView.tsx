      ...
      <div className="settings-layout-grid">
        <Card style={{ gap: 12, padding: 14 }}>
          ...
          <div className="settings-family-preview">
            ...
            {isOwner && isSupabaseConfigured() ? <ShadcnButton type="button" variant="ghost" size="icon" className="settings-icon-action" onClick={() => setManager('family')} style={{ color: colors.blue, backgroundColor: colors.subtle }} aria-label="Convidar membro"><UserPlus size={17} /></ShadcnButton> : null}
          </div>
        </Card>
        ...
        <Card style={{ gap: 12, padding: 14 }}>
          ...
          <div className="settings-maintenance-actions">
            <ConfirmAction
              ...
              trigger={<Button onPress={() => undefined} variant="danger"><Database size={15} /> Limpar cache</Button>}
            />
            <Button onPress={() => void signOut().then(() => router.replace('/entrar'))} variant="ghost"><LogOut size={15} /> Sair</Button>
          </div>
        </Card>
      </div>
              ...
              <RowItem
                ...
                right={isOwner && !member.isYou ? (
                  <ConfirmAction
                    ...
                    trigger={<ShadcnButton type="button" variant="ghost" className="settings-row-danger" style={{ color: colors.red, backgroundColor: colors.subtle }}>Remover</ShadcnButton>}
                  />
                ) : undefined}
              />
            ))}
      ...
      {manager === 'accounts' ? (
        <SettingsManagerModal title="Gerenciar contas" subtitle={`${accounts.length} conta(s) · ${cardCount} cartão(ões)`} onClose={() => setManager(null)}>
          <div className="settings-manager-toolbar">
            ...
            <ShadcnButton type="button" variant="ghost" className="settings-add-button" onClick={() => setShowAccountForm((visible) => !visible)} style={{ color: colors.blue, backgroundColor: colors.subtle }}><Plus size={15} /> Nova conta</ShadcnButton>
          </div>
          {showAccountForm ? (
            <div className="settings-create-account" style={{ backgroundColor: colors.subtle }}>
              ...
              <ToggleGroup value={[accountType]} onValueChange={(values) => { const nextType = values[0] as Account['type'] | undefined; if (nextType) setAccountType(nextType); }} className="chip-grid" aria-label="Tipo da conta">{accountTypes.map((item) => <ToggleGroupItem variant="outline" key={item.value} value={item.value} className="chip" style={{ backgroundColor: accountType === item.value ? colors.ink : colors.surface, color: accountType === item.value ? colors.bg : colors.ink, borderColor: colors.line }}>{item.label}</ToggleGroupItem>)}</ToggleGroup>
            ...
            {accounts.map((account) => (
              <div key={account.id} className="settings-account-item" style={{ borderColor: colors.line }}>
                <RowItem
                  ...
                  right={<div className="settings-account-actions"><ShadcnButton type="button" variant="ghost" className="account-edit-button" onClick={() => setEditingAccountId(editingAccountId === account.id ? null : account.id)} style={{ color: colors.blue, backgroundColor: colors.subtle }}><Pencil size={14} /> Editar</ShadcnButton><ConfirmAction title={`Excluir a conta “${account.name}”?`} description="As transações já registradas serão preservadas." onConfirm={() => handleDeleteAccount(account)} trigger={<ShadcnButton type="button" variant="ghost" className="account-delete-button" disabled={deletingAccountId === account.id} style={{ color: colors.red, backgroundColor: colors.subtle }} aria-label={`Excluir conta ${account.name}`}>{deletingAccountId === account.id ? <Spinner /> : <Trash2 size={14} />} Excluir</ShadcnButton>} /></div>}
                />
...
function SettingsCardHeader({ icon, title, detail, color, action, actionLoading = false, onAction }: { icon: React.ReactNode; title: string; detail: string; color: string; action?: string; actionLoading?: boolean; onAction?: () => void }) {
  return (
    <div className="settings-section-heading">
      ...
      {action && onAction ? <ShadcnButton type="button" variant="ghost" className="settings-card-action" onClick={onAction} disabled={actionLoading} style={{ color, backgroundColor: `${color}12` }}>{actionLoading ? <Spinner /> : action}</ShadcnButton> : null}
    </div>
  );
}
...
function SettingsManagerModal({ title, subtitle, onClose, children }: { title: string; subtitle: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="settings-manager-modal" style={{ backgroundColor: colors.bg, borderColor: colors.line }}>
        <header><div><DialogTitle id="settings-manager-title">{title}</DialogTitle><DialogDescription style={{ color: colors.muted }}>{subtitle}</DialogDescription></div></header>
        <Separator />
...
function ConfirmAction({ title, description, trigger, onConfirm }: { title: string; description: string; trigger: React.ReactElement; onConfirm: () => void | Promise<void> }) {
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      <AlertDialogTrigger render={<span className="contents" />}>{trigger}</AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction variant="destructive" onClick={() => void confirm()}>Confirmar</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  );
...
function AccountEditor({ account, accountTypes, onSave, onCancel }: {
  ...
  return (
    <div className="account-editor" style={{ backgroundColor: colors.subtle }}>
      ...
      <ToggleGroup value={[type]} onValueChange={(values) => { const nextType = values[0] as Account['type'] | undefined; if (nextType) setType(nextType); }} className="account-type-picker" aria-label="Tipo da conta">{accountTypes.map((item) => <ToggleGroupItem variant="outline" key={item.value} value={item.value} className="chip" style={{ backgroundColor: type === item.value ? colors.ink : colors.surface, color: type === item.value ? colors.bg : colors.ink, borderColor: colors.line }}>{item.label}</ToggleGroupItem>)}</ToggleGroup>
